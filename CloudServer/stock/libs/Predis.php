...
class Predis
{
    ...
    //初始化
    public function __construct($host,$port,$auth=null,$timeout=0,$type=1)
    {
        $this->_HOST = $host;
        $this->_PORT = $port;
        $this->_AUTH = $auth;
...
//        return $return;
    ...
    /**
     * 获取有序集合中的所有交易对
     * @param $ykey   有序集合中的key
     * @param int $begin
     * @param int $over
     */
    public function listGets($ykey,$begin = 0,$over = -1){
        $pairs = $this->_REDIS->zRevRange($ykey,$begin,$over);
        return $pairs;
    ...
    /**
     * 获取有序集合中的所有交易对，查询哈希表中交易对的信息并按顺序排序（管道操作）
     * @param $ykey   有序集合中的key
     * @param $hkey   哈希表的key
     * @param array $fields
     * @param int $begin
     * @param int $over
     */
    public function hMget($ykey,$hkey,$fields = array(),$begin = 0,$over = -1){
        $pairs = $this->_REDIS->zRevRange($ykey,$begin,$over);
     ...
     * @param int $scan
     */
    public function hscanGet($pattern,$count){
...
//        return $this->_REDIS->hScan($hash,0,$scans);
//        return $this->_REDIS->hScan($hash,$scan);
//        return $this->_REDIS->scan($hash,$scans);
    }
     ...
     */
    public function tranWatch($name)
    {
        return $this->_REDIS->watch($name);
    }
    /**
     * 开始进入事务操作
     * @param $return object 事务对象
     */
    public function tranStart($parm=null)
    {
        if ($parm == null){
            return $this->_REDIS->multi();
        }else{
            return $this->_REDIS->multi($parm);
        }
