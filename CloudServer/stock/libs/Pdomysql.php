...
//    );
    ...
    //获取对象,同时初始化DB 对象
    public static function getInstance($type,$host,$user,$pass,$db,$port,$charset='UTF8',$pconnect=false,$debug=true) {
        self::$config['dbtype'] = $type;
        self::$config['dbhost'] = $host;
        self::$config['dbuser'] = $user;
        self::$config['dbpass'] = $pass;
        self::$config['databases'] = $db;
        self::$config['dbport'] = $port;
        self::$config['charset'] = $charset;
        self::$config['pconnect'] = $pconnect;
    ...
    public static function connect() {
        try {
            ...
            var_dump("Connect Error Infomation:" . $e->getMessage());
//            die("Connect Error Infomation:" . $e->getMessage());
    ...
    private static function errorfile($sql) {
        $errorfile = WEB_ROOT.'/logs/dberrorlog.php';
        $sql = str_replace(
            array(
                "\n",
                "\r",
